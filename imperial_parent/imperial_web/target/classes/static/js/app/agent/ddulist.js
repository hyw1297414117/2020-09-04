...
var ddulistTableDivWid = $("#ddulistTableDiv").width()*0.97+"px";
$(function() {
    $("#ddulistTable").bootstrapTable({
     ...
     * 标记父子表中被选中的父表tr
     * */
    $("#ddulistTable tbody").on('click',function(){
    	var plusiconObject = $("#ddulistTable").find("tr").find("td").find(".glyphicon-plus");
    	var minusiconObject = $("#ddulistTable").find("tr").find("td").find(".glyphicon-minus");
...
function InitBagNumberTable (mainNoIndex, fatRow, $fatDetail) {
	...
	dataMap.incoterms = 'DDU';
	var cur_table = $fatDetail.append('<div id="bagDiv'+mainNoIndex+'" style="width:'+ddulistTableDivWid+'"><table id="bagTable'+mainNoIndex+'"></table></div>').find('table');
...
function InitSubTable (mainNoIndex,bagNumberIndex, row, $thisDetail,mainOrderNo) {
	...
	$(cur_table).bootstrapTable({
    	id: "ddulistSunTable"+tableidIndex,
	...
	$('#ddulistSunTable'+tableidIndex).on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table load-success.bs.table", function () {
		var rows = $('#ddulistSunTable'+tableidIndex).bootstrapTable('getSelections');
		// 非多个禁用
...
function searchFun(){
	...
	$.operate.impAjax(prefix + "/getMainOrderNoList",params,function(json){
		$("#ddulistTable").bootstrapTable('load',json);//主要是要这种写法
	},function(){
...
function showCheckPage(){
	...
	var tableId = "ddulistSunTable"+operationDivId.substr(9); //获取点击按钮所属的tablle索引
	var rows = $("#ddulistSunTable"+operationDivId.substr(9)).bootstrapTable('getSelections');
