...
var submitsuccessTbDivWid = $("#submitsuccessTbDiv").width()*0.97+"px";
$(function() {
    $("#submitsuccessTable").bootstrapTable({
     ...
     * 标记父子表中被选中的父表tr
     * */
    $("#submitsuccessTable tbody").on('click',function(){
    	var plusiconObject = $("#submitsuccessTable").find("tr").find("td").find(".glyphicon-plus");
    	var minusiconObject = $("#submitsuccessTable").find("tr").find("td").find(".glyphicon-minus");
...
function InitBagNumberTable (mainNoIndex, fatRow, $fatDetail) {
	...
	dataMap.checkFlag = 1;
	var cur_table = $fatDetail.append('<div id="bagDiv'+mainNoIndex+'" style="width:'+submitsuccessTbDivWid+'"><table id="bagTable'+mainNoIndex+'"></table></div>').find('table');
...
function InitSubTable (mainNoIndex,bagNumberIndex, row, $thisDetail,mainOrderNo) {
	...
	dataMap.checkFlag = 1;
	$thisDetail.append('<div id="smsenable'+tableidIndex+'" style="width:'+submitsuccessTbDivWid+'">'+$("#operatBtTpl").html()+'</div>');
	var cur_table = $thisDetail.append('<div id="detailDiv'+tableidIndex+'" style="width:'+submitsuccessTbDivWid+'"><table id="submitsuccessTable'+tableidIndex+'"></table></div>').find('table');
	var tableid = 'submitsuccessTable';
	$(cur_table).bootstrapTable({
    	id: "submitsuccessTable"+tableidIndex,
	...
	$('#submitsuccessTable'+tableidIndex).on("check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table load-success.bs.table", function () {
		var rows = $('#submitsuccessTable'+tableidIndex).bootstrapTable('getSelections');
		// 非多个禁用
...
function searchFun(){
	...
	$.operate.impAjax("/shipperModule/impBasicData/getMainOrderNoList",params,function(json){
		$("#submitsuccessTable").bootstrapTable('load',json);//主要是要这种写法
	},function(){
