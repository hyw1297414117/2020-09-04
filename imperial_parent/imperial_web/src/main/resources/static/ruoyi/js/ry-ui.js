...
(function ($) {
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    impParams: options.impParams, 						//清关自定义参数
            ...
            queryParams: function(params) {
            	var curParams = {
                        ...
                        isAsc:          params.order,
                        impParams: table.options.impParams
            		};
            ...
            search: function(formId, tableId, data) {
            	...
            	params.queryParams = function(params) {
                    ...
                    var options;
                    if($("#typeSelect")!=null){
                    	options = $("#typeSelect option:selected");
                    }
        ...
        operate: {
            ...
            impget: function(url,data,callback) {
            ...
            //清关系统ajax封装 
            impAjax:function(url,data,successFun,errorFun){
            	$.ajax({
    		        type: "post",
    		        url: url,
    		        data: data, 
 ...
 * 清关系统switch开关
 * */
/*function switchRender(index){
	$('#smsenable'+index).find(".smsenable").bootstrapSwitch({  
		        onText:"search",  
		        offText:"search",  
		        onColor:"success",  
		        offColor:"default",  
		...
		            if(state==true){  
						$(".formIdDiv").slideDown();  //展示检索框
		            }else{ 
						$(".formIdDiv").slideUp();    //隐藏检索框
		            }
				} 
    });
    $('#smsenable'+index).find(".smsenable").bootstrapSwitch("size","small");  //设置开关大小
}*/
/**
 * 清关系统switch开关
 * */
function switchRender(){
	$('.smsenable').bootstrapSwitch({  
		        onText:"search",  
		        offText:"search",  
		        onColor:"success",  
		        offColor:"default",  
    ...
    $('.smsenable').bootstrapSwitch("size","small");  //设置开关大小
}
/**
 * 清关系统  重新注册冻结列的checkbox $element是下层table的jq对象 
 * */
function registCheckbox(tableid,index,$element) {
    var options = $element.bootstrapTable('getOptions')
    if (!options.singleSelect) {
        //全选
        $('#detailDiv'+index+' .left-fixed-table-columns').on('click', 'input[name="btSelectAll"]', function () {
            var fixedSelectItem = $('#detailDiv'+index+' .left-fixed-table-columns input[name="btSelectItem"]');
        	if ($(this).is(':checked')) {
        		fixedSelectItem.prop('checked', true);
        		fixedSelectItem.parent().parent().addClass('fixedTrbgColor');
            } else {
            	fixedSelectItem.prop('checked', false);
            	fixedSelectItem.parent().parent().removeClass('fixedTrbgColor');
            }
            $('#'+tableid+index).find('thead').find('input[name="btSelectAll"]').click();
        });
    }
    //点击冻结行事件
    $('#detailDiv'+index+' .left-fixed-body-columns').on('click', 'tbody tr', function () {
            var inputs = $('#detailDiv'+index).find('.left-fixed-body-columns').find('input[name="btSelectItem"]');
            //true为单选   false为多选
            if (options.singleSelect) {
                ...
                var num = 0;
                inputs.each(function () {
                	$(this).parent().parent().removeClass('fixedTrbgColor');
                    if ($(this).is(':checked')) {
                        num++;
                        $(this).parent().parent().addClass('fixedTrbgColor');
                    }
                });
                if (num == inputs.length) {
                    $('#detailDiv'+index).find('.left-fixed-table-columns').find('input[name="btSelectAll"]').prop('checked', true);
                } else {
                	$('#detailDiv'+index).find('.left-fixed-table-columns').find('input[name="btSelectAll"]').prop('checked', false);
                }
            }
            var thisindex = $(this).index();
            $('#'+tableid+index).find('input[name="btSelectItem"]').eq(thisindex).click();
        });
    //点击右边主表行事件
    $('#'+tableid+index).on('click', 'tbody tr', function () {
    	var inputs = $element.find('input[name="btSelectItem"]');
        //true为单选   false为多选
        if (options.singleSelect) {
            var checked = $(this).is(":checked");//记录点击的状态
            inputs.each(function () {
            ...
            if (num == inputs.length) {
            	$element.find('input[name="btSelectAll"]').prop('checked', true);
            } else {
            	$element.find('input[name="btSelectAll"]').prop('checked', false);
            }
        }
        var thisindex = $(this).index();
        //冻结列事件
        var fixedBtSelectItems = $('#detailDiv'+index+' .left-fixed-body-columns').find('input[name="btSelectItem"]');
        fixedBtSelectItems.eq(thisindex).click();
        if (fixedBtSelectItems.eq(thisindex).is(':checked')) {
            fixedBtSelectItems.eq(thisindex).parent().parent().addClass('fixedTrbgColor');
        }else{
        	fixedBtSelectItems.eq(thisindex).parent().parent().removeClass('fixedTrbgColor');
        }
    });
}
